...
public class AuthController {
    ...
    @PostMapping("/singin")
    public ResponseEntity<?> createAuthTokenForNewUser(@RequestBody JwtRequest authRequest) {
        if (!userService.findByUsername(authRequest.getUsername()).isPresent())
            userService.save(authRequest.getUsername(), authRequest.getPassword());
        else
            return new ResponseEntity<>(new MarketError(HttpStatus.BAD_REQUEST.value(), "Incorrect username"), HttpStatus.BAD_REQUEST);
