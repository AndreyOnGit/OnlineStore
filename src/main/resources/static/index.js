angular.module('app', []).controller('indexController', function ($scope, $http) {
    ...
    console.log('in $scope.changeAddress');
    console.log ($scope.delivery);
         $http.post(contextPath + '/api/v1/orders/change_address', $scope.delivery)
              .then(function (response) {
                   $scope.showOrder();
         });
    };
     ...
     $scope.tryToAuth = function () {
        $http.post(contextPath + '/auth', $scope.user)
            .then(function successCallback(response) {
                ...
                window.alert("Неверно введены данные для авторизации");
            });
    };

     $scope.tryToSingIn = function () {
        $http.post(contextPath + '/singin', $scope.user)
            .then(function successCallback(response) {
            $scope.tryToAuth();
            }, function errorCallback(response) {
                window.alert("Пользователь с таким именем существует");
            });
    };
